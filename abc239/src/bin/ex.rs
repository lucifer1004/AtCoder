...
fn dfs(n: usize, m: usize, memo: &mut HashMap<usize, Mint>) -> Mint {
    if let Some(&val) = memo.get(&m) {
        return val;
    }
    ...
    while r > 0 {
        ...
        let l = m / (q + 1);
        if q != m {
            ans += dfs(n, q, memo) * (r.min(n) - l.min(n));
        }
        r = l;
    }
...
fn main() {
    ...
    memo.insert(0, Mint::new(0));
    dfs(n, m, &mut memo);
    println!("{}", memo.get(&m).unwrap());
}
